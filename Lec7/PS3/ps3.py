...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        # Place dirt amount on each tile
        self.tile_dirt = {}
        for x in range(width):
            for y in range(height):
                self.tile_dirt[x,y] = dirt_amount
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        self.tile_dirt[x,y] -= capacity
        if self.tile_dirt[x,y] < 0:
            self.tile_dirt[x,y] = 0
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        cleaned_tiles = 0
        for tile in self.tile_dirt.keys():
            if self.tile_dirt[tile] == 0:
                cleaned_tiles += 1
    ...
    def is_position_in_room(self, pos):
        ...
        """
        if pos.get_x() <= self.width and pos.get_y() <= self.height\
            and pos.get_x() >= 0 and pos.get_y() >= 0:
            return True
        else:
            return False
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        x = random.uniform(0,self.get_width())
        y = random.uniform(0,self.get_height())
        return Position(x,y)
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        return self.get_width()*self.get_height()-len(self.furniture_tiles)
    ...
    def get_random_position(self):
        ...
        while not found_pos:
            ...
            found_pos = self.is_position_valid(rand_pos)
        return rand_pos
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        room = self.get_robot_room()
        robot_position = self.get_robot_position()
        robot_speed = self.get_robot_speed()
        robot_direction = self.get_robot_direction()
        capacity = self.get_robot_capacity()
        x = robot_position.get_x()
        y = robot_position.get_y()
        x_hat = math.sin(math.radians(robot_direction))
        y_hat = math.cos(math.radians(robot_direction))

        new_x = x+x_hat*robot_speed
        new_y = y+y_hat*robot_speed
        new_pos = Position(new_x, new_y)
        if room.is_position_valid(new_pos):
            self.set_robot_position(new_pos)
            room.clean_tile_at_position(new_pos, capacity)
        else:
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_y = y+y_hat*robot_speed
        new_pos = Position(new_x, new_y)
        if not self.gets_faulty() and room.is_position_valid(new_pos):
            self.set_robot_position(new_pos)
            room.clean_tile_at_position(new_pos, capacity)
        else:
            self.set_robot_direction(random.uniform(0,360))
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        # Initialize Room and Robots per Trial
        ...
        anim.done()
    mean = sum(time_steps)/num_trials
    return mean
...
print ('avg time steps: ' + str(run_simulation(50, 1.0, 1, 40, 40, 3, 1.0, 50, StandardRobot)))
